...
impl TryFrom<OutboundHysteria2> for Handler {
    ...
    fn try_from(value: OutboundHysteria2) -> Result<Self, Self::Error> {
        ...
        let opts = HystOption {
            ...
            ports: ports_gen,
            obfs,
            up_down: value.up.zip(value.down).map(|(u, d)| (u * 1_000_000, d * 1_000_000)),
            ca_str: value.ca_str,
            cwnd: value.cwnd,
...
mod tests {
    ...
    fn test_hysteria2_url_parse() {
        ...
        let outbound: OutboundHysteria2 = url_str.parse().expect("failed to parse hysteria2 url");
        ...
        let handler = Handler::try_from(outbound).expect("failed to build handler from outbound");
    ...
    async fn test_hysteria2_client_live_connection() {
        ...
        let handler = std::sync::Arc::new(
            Handler::try_from_url(url_str).expect("failed to parse and create hysteria2 handler")
        );
        handler.register_connector(GLOBAL_DIRECT_CONNECTOR.clone()).await;

        let sess = Session {
            destination: SocksAddr::Domain("www.baidu.com".to_string(), 80),
            ..Default::default()
        };
        let dummy_resolver = Arc::new(crate::app::dns::SystemResolver::new(true).unwrap()) as ThreadSafeDNSResolver;

        println!("Testing Hysteria2 client connection to www.baidu.com:80 via 192.168.10.106:60747...");
        ...
        stream.write_all(request).await.expect("Failed to write request to Hysteria2 stream");

        let mut response = Vec::new();
        stream.read_to_end(&mut response).await.expect("Failed to read response from Hysteria2 stream");

        let response_str = String::from_utf8_lossy(&response);
        println!("Received response from www.baidu.com:\n{}", &response_str[..response_str.len().min(500)]);
        assert!(
            response_str.contains("200 OK") || response_str.contains("302") || response_str.contains("baidu"),
    ...
    async fn test_hysteria2_client_live_https_connection() {
        ...
        println!("Testing Hysteria2 client HTTPS connection to https://www.baidu.com:443 via 192.168.10.106:60747...");
        let stream = handler
            .connect_stream(&sess, dummy_resolver)
            .await
            .expect("Failed to connect stream via Hysteria2");
        ...
        let tls_config = build_tls_client_config(verifier, None, None).expect("failed to build tls config");
        let connector = tokio_rustls::TlsConnector::from(Arc::new(tls_config));
        let domain = rustls::pki_types::ServerName::try_from("www.baidu.com").expect("invalid domain");

        let mut tls_stream = connector.connect(domain, stream).await.expect("Failed to establish TLS session over Hysteria2 stream");

        let request = b"GET / HTTP/1.1\r\nHost: www.baidu.com\r\nUser-Agent: curl/7.68.0\r\nConnection: close\r\n\r\n";
        tls_stream.write_all(request).await.expect("Failed to write request to Hysteria2 TLS stream");

        let mut response = Vec::new();
        tls_stream.read_to_end(&mut response).await.expect("Failed to read response from Hysteria2 TLS stream");

        let response_str = String::from_utf8_lossy(&response);
        println!("Received HTTPS response from www.baidu.com:\n{}", &response_str[..response_str.len().min(500)]);
        assert!(
            response_str.contains("200 OK") || response_str.contains("302") || response_str.contains("baidu"),
            "Unexpected response: {}", response_str
        );
