...
fn test_http_host_parser_various_methods() {
    let get_req = b"GET /index.html HTTP/1.1\r\nHost: example.org\r\nAccept: */*\r\n\r\n";
    assert_eq!(http::parse_http_host(get_req), Some("example.org".to_string()));

    let post_req = b"POST /submit HTTP/1.1\r\nHOST: sub.domain.com:8080\r\nContent-Length: 0\r\n\r\n";
    assert_eq!(http::parse_http_host(post_req), Some("sub.domain.com".to_string()));

    let connect_req = b"CONNECT static.cloudflare.com:443 HTTP/1.1\r\nhost: static.cloudflare.com:443\r\n\r\n";
    assert_eq!(http::parse_http_host(connect_req), Some("static.cloudflare.com".to_string()));
}
...
async fn test_sniffer_stream_tls() {
    ...
    let (domain, mut stream, override_dest) = sniffer.sniff_stream(&sess, Box::new(client)).await;
    ...
    assert_eq!(&buf[0..5], &[0x16, 0x03, 0x01, 0x00, (sample_len - 5) as u8]);
}
...
async fn test_sniffer_stream_http_with_override() {
    ...
    let (domain, _stream, override_dest) = sniffer.sniff_stream(&sess, Box::new(client)).await;
...
fn test_domain_matcher() {
    ...
    assert!(sniffer.matches_domain_list("www.google.com", &sniffer.config.force_domains));
}

#[test]
fn test_port_matcher() {
    let matcher = PortMatcher::new(vec![
        PortRange::Single(80),
        PortRange::Range(8080, 8088),
    ]);
