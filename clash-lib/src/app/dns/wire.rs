...
pub fn skip_dns_name(data: &[u8], pos: &mut usize) -> bool {
    loop {
        if *pos >= data.len() {
            return false;
        }
...
pub fn extract_ips_from_dns_response(response: &[u8]) -> Vec<IpAddr> {
...
pub fn extract_ips_with_ttl(response: &[u8]) -> Vec<(IpAddr, u32)> {
    let mut out = Vec::new();
    if response.len() < 12 {
        return out;
    }
...
/// Extract the minimum TTL across all answer records from a wire-format DNS response.
pub fn extract_min_ttl_from_dns_response(response: &[u8]) -> Option<u32> {
    if response.len() < 12 {
        return None;
    }
    let qdcount = u16::from_be_bytes([response[4], response[5]]) as usize;
...
/// Overwrite TTL fields on all answer records in a wire-format DNS response.
pub fn rewrite_dns_response_ttl(response: &mut [u8], new_ttl: u32) {
    if response.len() < 12 {
        return;
    }
    let qdcount = u16::from_be_bytes([response[4], response[5]]) as usize;
    ...
    for _ in 0..ancount {
        ...
        if rtype != 41 {
            // Non-OPT record
            response[pos + 4..pos + 8].copy_from_slice(&ttl_bytes);
        }
        let rdlength = u16::from_be_bytes([response[pos + 8], response[pos + 9]]) as usize;
        pos += 10 + rdlength;
    }
...
pub fn parse_dns_name_to_string(data: &[u8], pos: usize) -> Option<String> {
    ...
    loop {
        ...
        if len > 63 || cursor + 1 + len as usize > data.len() {
            return None;
        }
...
pub fn parse_dns_response_records(response: &[u8]) -> Vec<DnsRecordInfo> {
    ...
    for _ in 0..ancount {
        ...
        let name = parse_dns_name_to_string(response, name_start).unwrap_or_default();
        if pos + 10 > response.len() {
            break;
        }
        let rtype = u16::from_be_bytes([response[pos], response[pos + 1]]);
        ...
        let data = match rtype {
            1 if rdlength == 4 => {
                format!("{}.{}.{}.{}", response[pos], response[pos + 1], response[pos + 2], response[pos + 3])
            }
            28 if rdlength == 16 => {
                let mut octets = [0u8; 16];
                octets.copy_from_slice(&response[pos..pos + 16]);
            ...
            5 | 12 => {
                parse_dns_name_to_string(response, pos).unwrap_or_else(|| format!("<{} bytes>", rdlength))
            }
            _ => format!("<{} bytes>", rdlength),
        };

        out.push(DnsRecordInfo {
            name,
            rtype,
            ttl,
            data,
...
mod tests {
    ...
    #[test]
    fn test_rewrite_dns_response_ttl() {
        let mut resp = make_response(1, &[1, 2, 3, 4], 300);
        let before = extract_ips_with_ttl(&resp);
        assert_eq!(before[0].1, 300);
